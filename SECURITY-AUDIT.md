...
**Fixed, pass 3 (build + 11 vitest + lint + tsc clean):** M5 (booking_settings full-row exposure — public SELECT policy dropped; the 3 public booking routes now read settings through a SECURITY DEFINER `get_public_booking_settings(slug)` RPC keyed on a known slug, so anon can no longer enumerate every tenant's row) · L8 (the **500-path** raw-`error.message` schema-disclosure leaks genericized to `"Internal server error"` + `console.error` logging across 40 files / 68 sites; intentional 4xx validation messages left intact).
**Still open (product decision only):** H2 chat full fix (signed widget token + `web_chat_enabled` flag — schema/product feature) · L5 chat/init business-name leak (needs the same `web_chat_enabled` flag). Both require a new per-tenant "web-chat widget on/off" toggle in settings UI — a product feature, not pure hardening. H2's dangerous half (AI-cost abuse) is already closed by the per-IP rate limit.
**DB fixes live in** `20260628000001_security_hardening.sql` + `20260628000002_public_rls_hardening.sql` — **both APPLIED to the local DB and verified 2026-06-28**: as the anon role, a published tenant's services/availability/blocked_dates ARE visible (booking page works) while non-published tenants are excluded (`has_published_booking(random)=false`); notifications INSERT policy confirmed `user_id=auth.uid() OR service_role`. They will re-apply on the next clean `db push` to prod.
...
- **M5 (LOW)** `booking_settings` — `USING (booking_url_slug IS NOT NULL)` → full settings row incl. `user_id`. `:100` ✅ FIXED — `20260628000003_booking_settings_rpc.sql` drops the public policy and adds `get_public_booking_settings(slug)` (SECURITY DEFINER, slug-keyed, `REVOKE … FROM PUBLIC` + GRANT to anon/authenticated/service_role). The 3 public booking routes call the RPC instead of `select("*")`. Verified on local DB as anon: `get_public_booking_settings('sarah-mitchell')` → 1 row; bogus slug → 0; direct `SELECT … FROM booking_settings` → 0 (enumeration closed).
...
- **L8** DB error messages forwarded verbatim (`{ error: error.message }`) across ~30 routes → internal schema disclosure; violates CLAUDE.md convention. Return generic 5xx, log server-side. ✅ FIXED — all 68 **status-500** sites (40 files) now return `"Internal server error"` and `console.error("[API]", error)` server-side. Scoped deliberately to 500 paths; 4xx Zod/validation messages (intended, user-facing) were left as-is.
